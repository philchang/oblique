...
\end{equation}
where $\epsilon$ is the simplified dielectric function, and for each species $\omega_{p,s}^2\equiv 4\pi e^2 n_s/m_e$ is the plasma frequency, $n_s\equiv\int f_{0s} d^3\!p$ is the number density, and $F_s\equiv f_{0s} / n_s$ is the normalized \ab{background} distribution function.
...
beam particles are \ab{resonant \sout{resonance}} with the unstable wave over a timescale much longer
than the growth time \ab{\sout{as stated above}}, i.e., the beam particles
...
the plasma (background or beam) does not respond to the disturbance\ab{; instead, \sout{. Instead,}}
...
regimes of instability, and the delineation between \ab{them. \sout{these two regimes.}}
...
For the more general case where $Z_x\ne 0$, this becomes the oblique instability studied by \citet{Bret-Grem-Beni:10}  Indeed for $\gamma \gg 1$ and \ab{$Z_x\gg1$ \sout{$Z_x \rightarrow \infty$}}, the growth rate approaches oblique growth rate:
\begin{equation}
...
\subsubsection{Sub-maximal instabilities \ab{[There should be either more or fewer subsubsections]}}
...
The growth rate expressed in Equation (\ref{eq:growth rate reactive}) is in the reactive (or hydrodynamic) regime as the dispersion relation (Equation (\ref{eq:dispersion reactive})) could have been derived from the fluid equations.  Here all the particles participate in the instability.  However, kinetic theory \ab{marks \sout{masks}} another regime of the instability, where only a fraction of the particle participate in the instability, i.e., the kinetic regime.  We now derive the growth rate of the instability in the kinetic regime.
...
For the beam plasma, we \ab{again} adopt the Maxwell-J{\"u}ttner distribution (\ab{Equation \sout{eq.}}~(\ref{eq:distribution function beam})).
...
%\end{equation}
Inserting these into the dispersion relation\ab{, Equation} (\ref{eq:dispersion relation}), we find
...
\end{aligned}
\end{equation}
where $\mu = m_e c^2/k_B T_b$, $\gph = \left(1 - \vph^2/c^2\right)^{-1/2}$, $\vph = \omega/k$ is the phase velocity of the wave, $v_{b,z'}$ is the velocity of the beam oriented along the wavevector, $v_{b,x'}$ is the velocity of the wave perpendicular to the wavevector, $\cG$ is the Lorentz factor of a beam particle in a frame comoving with wave at the phase velocity \ab{[While this is true, I think $\cG'$ is a little different -- it relates to the transverse boosted frame.]}, and $\Gamma_0 \equiv \omega_p \gammabeam (n_b/n_t) (m_e v_B^2/k_B T_b)$ is the typical maximum growth rate.  This specializes to the beam-plasma growth rate if we take $v_{b,x'} = 0$, which gives:
\begin{equation}
...
rate reaches it maximum value $\sin\theta \gg 1/\gammabeam$, i.e., at an oblique angle \ab{[I thought it was for $\theta\gtrsim1/\gammabeam$]}.  Note that at $\sin\theta = 0$, we recover the beam-plasma instability.  Moreover the maximum growth rates vary little and are
...
In Paper I \ab{[Paper I is nowhere defined]}, we compared the cold plasma growth rates of the oblique and beam-plasma instabilities and noted that the oblique cold growth rate is larger.  While we also noted \ab{that the} oblique instability was in the kinetic regime in Paper I, we made no effort to study the regime of instability of the beam-plasma case.  Here we have shown that while the oblique growth rate is kinetic, the beam-plasma rate is marginally reactive.  We have also shown that the kinetic growth rates for both the oblique and beam-plasma instabilities are similar.  As a result, both instabilities (beam-plasma and oblique) have similar growth rates for the case of  pair beams from TeV blazers. \ab{[Where did we show that the beam-plasma instability is marginally reactive? Does this belong in the conclusions?]}
...
To begin let us return to the reactive instability.  For the growth rate of the reactive instability \ab{in Equation} (\ref{eq:growth rate reactive}) to be valid, the velocity dispersion \ab{must be vanishingly small. Quantitatively, this limit requires \sout{$\rightarrow 0$.   To reach this limit, we demand}}
\begin{equation}
 \kvec\cdot\Delta \vel \ll \omega - \kvec\cdot\vel_b.
\end{equation}
Taking $\kvec$ to be $\omega_{p,t}/\vel_b$, so that the RHS is $\approx \Gamma$, we find
\ab{[While it is true that $\omega-\omega_{p,t}\approx\Gamma$, this is not fully fleshed out in the main text; rather it is an inline comment in Appendix B.  Thus, this is likely to be a confusing part.  Also, if $\kvec=\omega_{p,t}/\vel_b$ then isn't hte LHS $\Delta v_\parallel/v_b$.]}
...
\end{equation}
where we have dropped constant factors of order unity and assumed that $Z_x \sim O(1)$.  Hence, this defines the upper limit on the velocity dispersion of the plasma for the cold-plasma approximation to hold and, hence, the range of validity for the reactive oblique growth rate \ab{Equation} (\ref{eq:growth rate reactive}).  For $Z_x \ll \gamma^{-2}$, we recover the condition for the relativistic, reactive beam-plasma instability:
...
\end{eqnarray}
Hence, the conditions for the reactive regime for the oblique mode (\ab{\sout{eq.} Equation}~(\ref{eq:oblique reactive regime})) and beam-plasma mode (\ab{\sout{eq.} Equation}~(\ref{eq:bp reactive regime})) can be reduced to 
