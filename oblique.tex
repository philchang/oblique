...
f = \frac{n_b m_e c^2}{4\pi\gammabeam k_B T K_2(m_ec^2/k_B T )m_e^3c^3} \exp\left(-\frac {\gammabeam(E - \vel_b\cdot \bmath{p})} {k_B T}\right)
\end{eqnarray}
...
Taking the limit of Equation (\ref{eq:distribution function beam}) as $k_BT  \rightarrow 0$, for a target plasma $v_0=0$ and a beam plasma $v_0=\vbeam$\footnote{That is, we set $F_s(\bmath{p})=\delta^3(\bmath{p}-\bmath{p}_{0s})$ where $\bmath{p}_{0s}\equiv \gamma_0 m_e v_0 \hat{\bmath{z}}$ is the momentum associated with $v_0$.}, we find
...
\end{aligned}
\end{equation}
where $\Gamma_0 \equiv \omega_p \gammabeam (n_b/n_t) (m_e v_B^2/k_B T)$ is the typical maximum growth rate.  This specializes to the beam-plasma growth rate if we take $v_{b,x'} = 0$, which gives:
\begin{equation}
...
\caption{Oblique growth rate maximized over $\vph$ as a function of
 $\sin\theta$ and $k_BT/m_e c^2$, where $\theta$ is the angle between the beam direction and wave vector for $\gammabeam = 10^6$. The maximum growth rate occurs when
  $\sin\theta \gg1/\gammabeam$. \cp{[ Can you please add $k_B$ instead of $k$ to the axis label and $\Gamma/\Gamma_0$ at the color bar? In the paper, the notation $T$ is used instead of $T_b$ here -- can you unify this?]}}\label{fig:ObliqueGMZ}
\end{figure}
...
\caption{Oblique kinetic growth rate (left) and beam-plasma growth rate (right),
  ...
  $k_BT/m_ec^2$ and $\gammabeam-1$.  Unlike the beam-plasma case on the right, in the case of the
...
\begin{equation}
\frac{\Delta v_{\perp}^2}{c^2} \approx \frac {k_B T}{\gammabeam^2 m_e c^2 },
\end{equation}
and 
\begin{equation}
\frac{\Delta v_{\parallel}^2}{c^2} \approx \frac {k_B T}{\gammabeam^4 m_e c^2 },
\end{equation}
where $T$ is measured in the COM frame of the beam.  

Applying these results, we find
\begin{eqnarray}
\frac{\Delta v_{\perp}}{\vbeam} &\sim&  \gammabeam^{-1}\sqrt{\frac{k_BT}{m_ec^2}},\\
\frac{\Delta v_{\parallel}}{\vbeam} &\sim& \gammabeam^{-2}\sqrt{\frac{k_BT}{m_ec^2}}.
\end{eqnarray}
...
1 \ll \left\{
 \begin{array}{cl}
 ({k_BT}/{m_ec^2})^{-1/2}\gammabeam^{2/3}\left({n_b}/{n_t}\right)^{1/3} & \textrm{oblique} \\
({k_BT}/{m_ec^2})^{-1/2}\gammabeam\left({n_b}/{n_t}\right)^{1/3} & \textrm{beam plasma} 
\end{array}
\right..
\end{equation}
...
In Figure \ref{fig:rates}, we plot the differential rate of pair production as a function of the COM energy of the electron (and positron) for a photon energy of $E_{\rm ph} = 0.3$ (dotted line), $1$ (solid line), $3$ (dash-dotted line) and $10\,\TeV$ (dashed line).  We use the COM energy in this case because in the COM frame, the energy of the EBL and TeV photon is the same and is $E_{\rm COM} = \sqrt{E_{\rm ebl}E_{\rm ph}}$ and $\theta \approx \pi - \sqrt{E_{\rm ebl}/E_{\rm ph}} \approx \pi$.  As a result, we can ignore the dependence on $\theta$ on the energy of the electron in the COM frame (eq.[\ref{eq:threshold}]) and set $1-\cos\theta \approx 2$.\cp{ [What do you mean by $\theta\to\cos\theta$: this is not clear!]}\phil{[hope this clarifies it.]}
...
\begin{equation}
 \dbetabarLABperp \approx \frac{\dbetabarCMperp}{\gammabeam^2} \approx \frac {k_B T}{\gammabeam^2 m_e c^2 }.
\end{equation}
...
\begin{equation}
 \dbetabarLABpara \approx \frac{\dbetabarCMpara}{\gammabeam^{4}}  \approx \frac {k_B T}{\gammabeam^4 m_e c^2 },
\end{equation}
...
\begin{eqnarray}
F_B &=& \frac{m_e c^2}{4\pi\gammabeam k_B T K_2(m_ec^2/k_B T )m_e^3c^3} \exp\left(-\frac {\gammabeam(E - v_{b,z'} p_{z'} - v_{b,x'} p_{x'})} {k_B T}\right)\\
%F_b &=& \frac{1}{4\pi\gammabeam m^2 T K_2(m/T)} \exp\left(-\frac {\gammabeam(E - \betabeamz p_{z'} - \betabeamx p_{x'}}{T}\right),\label{eq:dist1}\\
&=& \frac{\mu m_e^{-3}c^{-3}}{4\pi\gammabeam K_2(\mu)}  \exp\left(-\frac {\gammabeam\gz(c^2 - v_{b,z'}v_{z'})E_\perp)} {k_BT c^2}\right)\exp\left(\frac{\gammabeam v_{b,x'} p_{x'}}{k_BT}\right),\label{eq:dist2}
\end{eqnarray}
...
&=
\left.
-\frac{k\gammabeam}{k_BT} \left( \frac{p_{z'} c^2}{E} - v_{b,z'}\right)
F_b
\right|_{\rm pole}\\
&=
-
...
} \exp\left(-\frac {\gammabeam\gph(c^2 - v_{b,z'}v_{\rm ph})E_\perp)} {k_BT c^2}\right)\exp\left(\frac{\gammabeam v_{b,x'} p_{x'}}{k_BT}\right).
\end{aligned}
\end{equation}
%where we have used
%\begin{equation}
...
\label{eq:OGgen}
\end{equation}
where $\Gamma_0 \equiv \omega_p \gammabeam (n_b/n_t) (m_e v_B^2/k_B T)$ is the typical maximum growth rate.
