...
These constraints preclude any dramatic rise in numbers of source by \ab{\sout{$z\sim1$} $z\approx1$}--$2$ that is seen in the quasar
...
center of momentum (COM \ab{[Should we use CM instead for compactness? Otherwise ``of'' is getting a big promotion!]}) frame, i.e., $2 E E_{\rm ebl}(1-\cos\theta) \geq 4
m_e^2 c^4$, where $\theta$ is the relative angle of propagation in the lab
frame. As a result, an $\epm$ pair can be produced with Lorentz factor \ab{\sout{$\gamma = \left(1-v^2/c^2\right)^{-1/2} \simeq E/2m_e c^2$} $\gamma = \left(1-v^2/c^2\right)^{-1/2} \approx E/2m_e c^2$}, where $v$ is the velocity of the pairs \citep{Goul-Schr:67}.  Here, we discuss the distribution function of the
...
\frac{\Delta v_{\perp}^2}{c^2} \approx \frac {2 k_B T_b}{\gammabeam^2 m_e c^2 }\ab{%\sout{,}
\quad\text{and}\quad}
...
We now adopt perturbations of the form \ab{\sout{$\delta \sim \exp\left(i \kvec\cdot\rvec - i \omega t\right)$} $\delta \propto \exp\left(i \kvec\cdot\rvec - i \omega t\right)$} and without loss of generality assume that $\kvec = (k_x, 0, k_z)$, where $k_z$ is along the beam direction.  Linearizing the Vlasov-Maxwell equations then leads to the dispersion relation:
...
\end{equation}
For \ab{\sout{$k_\perp \sim \omega_p/v_b$} $k_\perp \approx \omega_p/v_b$}, this gives
...
\end{equation}
where $r_e = e^2/m_e c^2$ is the classical electron radius and $v_e$ is the electron velocity in the COM frame.  To find $v_e$, we use the energy of the electron in the COM frame, $E_{\rm e, COM}$, which is 
\begin{equation}
\label{eq:threshold}
...
In Figure \ref{fig:rates}, we plot the differential rate of pair production as a function of the COM energy of the electron (and positron), $E_{e,COM}$ for a photon energy of $E_{\rm ph} = 0.3$ (dotted line), $1$ (solid line), $3$ (dash-dotted line) and $10\,\TeV$ (dashed line).  
%We use the COM energy in this case because in the COM frame, the energy of the EBL and TeV photon is the same and is $E_{\rm ph, COM} = \sqrt{E_{\rm EBL}E_{\rm ph}}$ and $\theta_{\rm COM} \approx \pi - \sqrt{E_{\rm EBL}/E_{\rm ph}} \approx \pi$; in the COM frame the VHEGR and EBL photon have the same energy but opposite momentum. \phil{A rough order of magnitude calculation bears this out.  We begin by noting that $\tan\theta = p_{\rm EBL,\perp}/p_{\rm EBL, \parallel}$, where $p_{\rm EBL,\perp}$ and $p_{\rm EBL,\parallel}$ is the perpendicular and parallel components of the EBL photon's momentum relative to the TeV photon's momentum.  Boosting to the COM frame, the parallel component of the EBL's photons momentum is $p_{\rm EBL, COM,\parallel} \approx -E_{\rm ph, COM}/c$, while the perpendicular momentum is conserved, $p_{\rm EBL, COM,\perp} = p_{\rm EBL, \perp}$.  As a result, the angle in the COM frame is 
%\begin{equation}
%\tan\theta_{\rm COM} \approx \frac{p_{\rm EBL, COM,\perp}}{p_{\rm EBL, COM,\parallel}} \sim -\sqrt{\frac{E_{\rm EBL}}{E_{\rm ph}}} \approx -10^{-3} \left(\frac{E_{\rm EBL}} {1\,{\rm eV}}\right)^{1/2}\left(\frac{E_{\rm ph}}{1\,{\rm TeV}}\right)^{-1/2}
%\end{equation}
%}
...
%Second, the effect of the EBL spectrum on the distribution of electron COM energies is also seen in the breaks in the distribution.  For instance, the cutoff in $d\Gamma_{\pm}/dE_{\rm EBL}$ above $E_{\rm e, COM}/m_e c^2 \approx 3$ for $E_{\rm ph} = 1\,\TeV$ is due to the cutoff in the EBL spectrum above $10\,\eV$.  Additionally, the break in $d\Gamma_{\pm}/dE_{\rm EBL}$ at the same position for $E_{\rm ph} = 10\,\TeV$ is due to change in the EBL spectrum at $1\,\eV$.  
...
%&&\ab{\times}\left(\frac{E L_E}{10^{45}\erg\,\s^{-1}}\right)^{1/3}
...
We see from our reactive (\ref{eq:generic reactive regime}) and kinetic (\ref{eq:generic kinetic regime}) conditions, that the oblique instability always exists in the kinetic regime, but the beam plasma instability is in the reactive regime for $z\gtrsim 1$, for sufficiently cold beams $k_BT_b/m_e c^2 \lesssim 0.5$, which occurs for $E_{\rm ph} \lesssim 0.3\,\TeV$, or for large $\gamma$, which occurs for \ab{\sout{$E_{\rm ph} \sim 10\,\TeV$} $E_{\rm ph} \approx 10\,\TeV$} at $z=0$.  
...
so therefore,
\begin{equation}
  \fLAB\ab{[}\ppos_{\rm L}(\ppos_{\rm COM},\pmom_{\rm COM}),\pmom_{\rm L}(\ppos_{\rm COM},\pmom_{\rm COM})\ab{]} = \fCM(E_{\rm COM}).
\end{equation}
...
%\kI \equiv \int d^2\!p_\perp E_\perp \exp\left(-\frac{\cG(E_\perp-w p_x)}{k_B T_b}\right)
%\end{equation}
\ab{\sout{and}} $\cG\equiv\gammabeam\gph(1-v_{b,z'}\vph/c^2)$\ab{,} and
...
%d p_x d p_y &=& \frac{E_\perp}{E'_\perp} d p'_x d p'_y.
%\end{eqnarray}
Inserting this into Equation (\ab{\sout{\ref{eq:I-integral}} \ref{eq:Rogen I-integral}}), we find
\begin{equation}
\begin{aligned}
\kI &= 
