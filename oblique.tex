...
\begin{multline}
  ...
  =
  1 - \sum_s \frac{\omega_{p,s}^2}{k^2}\int F_s \frac{k^2 - (\kvec\cdot\betavec)^2}{\gamma(\omega - \kvec\cdot\vel)^2} d^3p 
  =
  0\,.
...
For the beam plasma, we adopt the Maxwell-Juttner distribution 
...
%\end{equation}
\begin{equation}\label{eq:dispersion kinetic}
 1 - \frac{\omega_{p,t}^2}{k^2}\int F_t\frac{k^2 - (\kvec\cdot\betavec)^2}{\gamma(\omega - \kvec\cdot\vel)^2} d^3p + \frac{m_e\omega_{p,b}^2}{k^2}\int \frac{\kvec\cdot\gradp F_b}{\omega - \kvec\cdot\vel}d^3p  = 0,
\end{equation}
...
\begin{equation}\label{eq:cross section}
 \sigma = \frac {1}{2} \pi r_e^2\left(1-\beta_e^2\right)\left[\left(3-\beta_e^4\right)\ln\frac{1+\beta_e}{1-\beta_e} - 2\beta_e\left(2-\beta_e^2\right)\right],
\end{equation}
where $r_e = e^2/m_e c^2$ is the classical electron radius and $\beta_e$ is the electron velocity in the COM frame.  To find $\beta_e$, we use the energy of the electron in the COM frame, $E_e$, which is 
\begin{equation}
\label{eq:threshold}
 E_e =  \frac{m_e c^2}{1-\beta_e^2} = \sqrt{\frac{1}{2}{E_{\rm ebl} E_{\rm ph}\left(1-\cos\theta\right)}}.
\end{equation}
...
\begin{eqnarray}
 \int F_t\frac{k^2 - (\kvec\cdot\betavec)^2}{\gamma(\omega - \kvec\cdot\vel)^2} d^3p &\approx& k^2
 \int F_t \left(\frac {1}{\omega^2} + \frac {2\kvec\cdot\vel}{\omega^3} + \frac{3 (\kvec\cdot\vel)^2}{\omega^4}\right) d^3p \nonumber \\
&\approx& \frac {k^2}{\omega^2}\left(1 + 3 {k^2\lambda_D^2}\right),\label{eq:dispersion-real-part}
\end{eqnarray}
...
This frame moves with a velocity, $\vel_{\rm ph} = \omega_k/k \zphat$.  With an eye toward computing the residue in (??), we define $p_z=\gph\vph\epsilon_\perp$, $\epsilon=\gph\epsilon_\perp$, and
...
&=&\frac{1}{
\pi\gamma_B m^2 T K_2(m/T)} \exp\left(-\frac {\gammabeam\gammaphase(1 - \betabeamz\betaphase)\epsilon_\perp)} {T}\right)\exp\left(\frac{\gammabeam\betabeamx p_{x'}}{T}\right),\label{eq:dist2}
\end{eqnarray}
...
- k \left(
  \frac{1}{\epsilon}
  -
  \frac{p_{z'}^2}{\epsilon^3}
\right)
\right|_{\rm pole}
=
- \frac{k}{\gph^3 \epsilon_\perp}\,,
\end{equation}
and 
\begin{equation}
\begin{aligned}
...
%=
\left.
-\frac{k\gamma_b}{T} \left( \frac{p_z}{\epsilon} - v_{bz}\right)
F_b
\right|_{\rm pole}\\
&=
-
\frac{
  k
  (\vph-v_{bz}) 
  e^{-\gamma_b\gph(1-v_{bz}\vph)\epsilon_\perp/T}
  e^{\gamma_b v_{bx} p_x/T}
}{
  4\pi m^2 T^2 K_2(m/T)
}\,,
\end{aligned}
\end{equation}
Note that the evaluation of the derivative is easiest using equation (\ref{eq:dist1}).
%where we have used
%\begin{equation}
%\left.
...
where
\begin{equation}
\kI \equiv \int d^2\!p_\perp \epsilon_\perp e^{-\cG(\epsilon_\perp-w p_x)/m},
\end{equation}
and $\cG\equiv\gamma_b\gph(1-v_{bz}\vph)m/T$ and
$w\equiv \gamma_b v_{bx} m/\cG T\le 1$.  This latter inequality is guaranteed as 
\begin{equation}
\cG \epsilon_\perp - \gamma_b v_{bx} p_x = \gamma_b(E - v_{bz} p_z - v_{bx}p_x)|_{v_{bz}=v_{\rm ph}} > 0
\end{equation}
...
Thus, we define $p_x'=\gamma_w(p_x-w \epsilon_\perp)$ and $p_y'=p_y$.  With this change in varables, we find:
\begin{eqnarray}
...
\kI
&= 
\int d^2\!p'_\perp \frac{\epsilon_\perp^2}{\epsilon'_\perp} e^{-\cG'\epsilon'_\perp/m}\\
%&=
...
\kI
=
\frac{2 \pi \gamma_w^2 m^3}{\cG'^3} \left[
